...
src/
  ...
  design-system/components.tsx  # primitive UI yang dipakai ulang
  design-system/tokens.ts       # helper token layout/focus/theme id
  design-system/theme.css       # CSS variable theme
public/
  genres/*.png                  # gambar kategori untuk GenresSection
```
...
- Edit gambar kategori di `public/genres`, lalu update path di `src/sections/GenresSection.tsx` kalau nama file berubah.
- Edit komponen reusable seperti `Button`, `ArtworkCard`, atau `MediaFrame` di `src/design-system/components.tsx`.
- Edit warna, font, radius, dan shadow global di `src/design-system/theme.css`.

## Section Notes

- `GenresSection` saat ini memakai 5 card kategori: `Lukisan`, `Patung`, `Relief`, `Kerajinan Seni`, dan `Dekorasi Artistik`.
